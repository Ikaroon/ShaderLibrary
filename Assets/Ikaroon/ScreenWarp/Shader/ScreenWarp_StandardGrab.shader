Shader "Ikaroon/Screen Warp/Standard Grab"
{
	Properties
	{
		...
		_GrabColor("Grab Color", Color) = (1,1,1,1)
		_GrabOpacity("Grab Opacity", Range(0,1)) = 1.0
		_GrabOffset("Grab Offset", Float) = 1.0
	...
	SubShader
	{
		...
		void surf (Input IN, inout SurfaceOutputStandard o)
		{
			...
			worldPos = IN.worldPos - IN.worldNormal * _GrabOffset;
			screenPos = ComputeScreenPos(mul(UNITY_MATRIX_VP, float4(worldPos.xyz, 1)));
			screenPos = screenPos / screenPos.w;
			float4 backgroundColor = tex2D(_BackgroundTexture, screenPos.xy) * _GrabColor;
			float remainder = 1 - _GrabOpacity;
			float opacity = _GrabOpacity + fresnel * remainder;
			col.rgb = lerp(col.rgb, backgroundColor.rgb, opacity);
